...
// le capteur renvoie l'info en Volts (masi 3V3 analogpin, capteur 5V pour 100 hPa)
  // capteur MPXV5010 Pdiff normalisée à 100hPa pour 5V
...
#define VALVE_IN_MAX 140 //valeur fermeture Vanne IN (90 fermé total)
#define VALVE_IN_MIN 0 // valeur ouverture Vanne IN (ouverte)
#define VALVE_OUT_MAX 140   // valeur Maxi vanne OUT
...
offset_Capteurs();
          ...
          inValve.write(VALVE_IN_MIN);    // valve IN OPEN = on gonfle !
          outValve.write(VALVE_OUT_MAX);  // valve OUT CLOSED
          delay (5000); // on laisse établir 30 hPa
          inValve.write(VALVE_IN_MAX);    // valve IN CLOSED = on retient son souffle!
                                          // là l'opérateur coupe la turbine
}

void loop() {
    delay (100);
    pression_A7=adc->adc0->analogRead(A7)-pression_offset;  // press diff patient, on tient compte de l'offset du capteur
...
{
unsigned char i;
// valve IN fermée, valve OUT ouverte, on élimine la pression dans le circuit
          inValve.write(VALVE_IN_MAX);    // valve IN CLOSED
          outValve.write(VALVE_OUT_MIN);  // valve OUT OPEN
          delay(5000); // et on laisse tomber la pression
